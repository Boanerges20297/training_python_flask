...
class BarbeiroServico(db.Model):
    ...
    barbeiro_id = db.Column(db.Integer, db.ForeignKey('barbeiros.id', ondelete='CASCADE'), nullable=False)
    servico_id = db.Column(db.Integer, db.ForeignKey('servicos.id', ondelete='CASCADE'), nullable=False)

    def __repr__(self):
        return f'<BarbeiroServico barbeiro={self.barbeiro_id} servico={self.servico_id}>'
