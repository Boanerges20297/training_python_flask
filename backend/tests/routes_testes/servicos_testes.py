...
def servico_com_type_errado_no_payload_teste():
    ...
    try:
        ...
        print(
            "- Criando serviço com payload usando tipo de dado errado para o campo preco"
        )
        response = requests.post(
            f"{url_base}/criar-servico",
            json={
        ...
        if response.status_code == 400:
            ...
            print(f"Resposta da API: {response.json()}")
            relatorio["criacao_servico_com_payload_invalido"][
...
def campos_faltando_no_payload_teste():
    ...
    try:
        print("- Iniciando teste de criação de serviço com payload inválido")
        print(
            "- Criando serviço com payload inválido sem o campo barbeiro_id para o teste"
        )
        response = requests.post(
            f"{url_base}/criar-servico",
            json={
