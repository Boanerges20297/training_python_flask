...
def criar_barbeiro_teste():
    ...
    try:
        ...
        response = requests.post(f'{url_base}/criar-barbeiro', json={
...
def barbeiro_com_type_errado_no_payload_teste():
    ...
    try:
        ...
        print("- Criando barbeiro com payload usando tipo de dado errado para o campo telefone")
        response = requests.post(f'{url_base}/criar-barbeiro', json={
            'nome': 'João Teste',
            'especialidade': 'Degrade',
        ...
        if response.status_code == 400:
            ...
            print(f"Resposta da API: {response.json()}")
            relatorio['criacao_barbeiro_com_payload_invalido']['campos_com_type_errado'] = True
            print("✅ Passou no teste")
        else:
        ...
        print("❌ Não passou no teste")
        print("-----------------------")
        relatorio['criacao_barbeiro_com_payload_invalido']['campos_com_type_errado'] = False
...
def barbeiro_com_campo_desconhecido_no_payload_teste():
    ...
    try:
        print("- Iniciando teste de criação de barbeiro com payload inválido")
        print("- Criando barbeiro com payload recebendo campo desconhecido para o teste")
        response = requests.post(f'{url_base}/criar-barbeiro', json={
            'nome': 'João Teste',
            'especialidade': 'Degrade',
        ...
        if response.status_code == 400:
            print('- Barbeiro não foi criado')
            print(f"Resposta da API: {response.json()}")
            relatorio['criacao_barbeiro_com_payload_invalido']['campo_desconhecido'] = True
            print("✅ Passou no teste")
        else:
            raise Exception("O barbeiro foi criado indevidamente ou houve outro erro.")
...
def editar_barbeiro_teste():
    ...
    try:
        ...
        response = requests.patch(f'{url_base}/editar-barbeiro/{barbeiro_id_teste}', json={
            'nome': 'João Barbeiro Editado',
            'especialidade': 'Corte Social'
        })
